...
def print_accuracy(accuracies, end_time):
	...
	filename = "results/" + "_".join([str(num_layers), str(Num_Points), str(square_Vol_Mu), str(square_Sigma), str(circle_Sigma), str(num_Nodes), str(num_epochs)]) + "_Testing.csv"
	f = open(filename, "w")
	print("Writing to " + filename)

	f.write("layers = " + str(num_layers) + "\n")
	f.write("points = " + str(Num_Points) + "\n")
	f.write("Square Volume Mu = " + str(square_Vol_Mu) + "\n")
	f.write("Square Volume Sigma = " + str(square_Sigma) + "\n")
...
def print_accuracy_Training(accuracies, end_time):
        ...
        filename = "results/" + "_".join([str(num_layers), str(Num_Points), str(square_Vol_Mu), str(square_Sigma), str(circle_Sigma), str(num_Nodes), str(num_epochs)]) + "_Training.csv"
        f = open(filename, "w")
        print("Writing to " + filename)
...
g=generator3.generator3()
square_Vol_Mu=float(sys.argv[1])
square_Sigma=float(sys.argv[2])
circle_Sigma=float(sys.argv[3])
...
AllData=g.generate_all_points(square_Vol_Mu,square_Sigma,circle_Sigma,Num_Points)
