...
class Trainer_np_Array():
	...
	def __init__(self):
		...
		#Placeholders to feed the trained dictionaries through.
                ...
                self.output=tf.nn.softmax(logits=output,name="Output Layer Activation Function")


		#Setting up the CostFunction operation and the Optimizer operation.
		CostFunction=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=output,labels=self.KnownLabels_Placeholder),name="Cost Function")
